import { DropCap } from '@/components/Book/DropCap';
import { Figure } from '@/components/Book/Figure';
import { Callout } from '@/components/Book/Callout';
import { DataTable } from '@/components/Book/DataTable';
import { QuizBlock } from '@/components/Book/QuizBlock';
import { TrainTestSplitDiagram, LeakageMap } from '@/components/Book/part4/PredictionDiagrams';

<DropCap>

A model that explains the past is not the same as a model that survives the future. The whole discipline of generalization is about that gap. We hold out a slice of history the model never sees, fit the model on the rest, and ask: when the held-out cases were brand-new to the algorithm, how well did it score them? If the answer is "almost as well as the training cases," the model has learned something the world will reward. If the held-out performance collapses, the model has memorized noise.

</DropCap>

This article is about three ideas. First, what the **train/test split** is and why it matters more than any single model choice. Second, how **cross-validation** generalizes the idea. Third — and this is where time is lost in practice — the catalog of **leakage** traps that quietly let information from the future creep into the past.

---

## The Executive Question

> If the model looks good on the data we used to train it, why might it still fail on next quarter's customers?

The deeply unsatisfying answer is: because the model can be brilliant at the historical task it was trained on and useless at the slightly different task production presents. Generalization is the test of whether those two tasks are close enough.

---

## The Train/Test Split

<Figure width="page-outset" caption="Figure 1. The minimum-viable evaluation. Historical labelled data is split before any model touches it; the training share is used to fit, the held-out share to grade. The grade is honest only if the test set was never used to choose anything about the model.">
  <div className="not-prose">
    <TrainTestSplitDiagram />
  </div>
</Figure>

In practice the split is rarely a single fixed cut. Three variants come up:

- **Random split.** Roughly 70/30 or 80/20. Works when rows are exchangeable and unrelated across time. Cheap, fast, the default for an early prototype.
- **Time-based split.** Train on data up to a cut-off date, evaluate on data after. The right default whenever the unit-by-time process has trend, seasonality, or drift — which is almost every business setting.
- **Group split.** Hold out entire groups (customers, stores, regions) rather than rows. Required when the model will be used to score *new* groups, not just future rows of old groups.

The choice should match the deployment story. A churn model scored weekly on the same customer base wants a time-based split. A pricing model that will be applied to listings the firm has never seen wants a group split where new listings are the held-out set.

---

## Cross-Validation

Cross-validation is what you reach for when one held-out slice doesn't give you a stable estimate of performance — typical when the dataset is small or noisy. The idea is to rotate the held-out role across many slices and average the resulting scores.

The point is not to squeeze more accuracy out of the model. It is to get a more **stable estimate** of how it would do on a slice it has not seen. A model with cross-validated AUC of 0.82 ± 0.01 is a different deployment risk than one with 0.82 ± 0.07, even though they have the same headline number.

---

## Leakage: Information From the Future

Leakage is when a feature contains information that would not have been available at decision time. It is the single most common reason a model with stellar offline metrics produces disappointing live results.

<Figure width="page-outset" caption="Figure 2. A leakage gallery for a churn task. Every feature has to be inspectable as of the score date — if its value is only knowable after the outcome occurs, it leaks.">
  <div className="not-prose">
    <LeakageMap />
  </div>
</Figure>

A few patterns to watch for:

- **Outcome-derived fields.** `cancellation_date`, `churn_reason_code`, `final_account_balance` — anything that exists because the outcome happened.
- **Time-window inconsistency.** Features computed over different windows for different rows. The classic version: `avg_spend_last_30_days` ending on the date of churn for churners and on the score date for non-churners.
- **Target encoding bleed.** Replacing a categorical with its mean target — fine if the mean is computed using only training rows, deadly if computed over the whole dataset.
- **Duplicate rows across the split.** The same customer appearing in both training and test with different timestamps, allowing the model to memorize their idiosyncrasies.
- **Future joins.** Joining a snapshot table that was rebuilt after the prediction date, so old rows carry today's metadata.

The remedy is procedural, not statistical: every feature should be reconstructible as of the score date by someone with access only to the warehouse and a clock, not by someone with the labelled outcomes.

<Callout variant="caveat" title="The 'too good to be true' rule">
  If a held-out AUC jumps by more than a few points after a new feature is added, suspect leakage before celebrating. Real signal accrues slowly; leakage arrives in one commit.
</Callout>

---

## Overfitting and Underfitting

Generalization can fail in two directions:

- **Overfitting.** The model is too flexible for the amount of signal in the data. Training error collapses; test error stays high or worsens. Visual symptom: dramatic gap between training and validation performance.
- **Underfitting.** The model is too constrained. Both training and test error stay high; the model misses real structure that a richer model would catch.

The bias–variance trade-off, which we'll revisit in [§15.4](/ch32-trees-ensembles), is the conceptual handle for this. A linear logistic model is biased toward simple relationships and rarely overfits. A deep tree or wide neural net is flexible and frequently does, unless regularized.

The diagnostic is always the same. If training error is much lower than test error, the model is overfitting. If both are bad, the model is underfitting or the features are insufficient.

---

## What Can Go Wrong

Beyond leakage proper, three procedural traps quietly defeat the test-set discipline:

1.  **Peeking.** Choosing hyperparameters or features by looking at test-set performance. The test set has, in effect, been turned into a (very noisy) training set. The remedy is a third **validation** slice — or cross-validation — used for choices, with the test set held inviolate until the final reporting.
2.  **Repeated submissions.** Iterating the model many times against the same held-out set selects, over many tries, for a model that happens to do well on that particular slice. This is the "leaderboard chasing" failure mode of Kaggle-style work.
3.  **Stale splits.** A split was made six months ago and the team has been iterating against it ever since. The world has moved; the test set hasn't. Refresh periodically with new held-out cohorts.

---

## Managerial Takeaway

The test set is a managerial artifact, not a technical one. Its job is to give the firm a credible estimate of how a model will behave on cases it has not yet seen. Two questions belong in every model review:

> *How was the held-out set constructed?* (Random / time / group.)
>
> *Could any feature in the model have been observed only after the outcome?*

A "no" to the second question, paired with a deployment-shaped answer to the first, buys most of the confidence a managerial sponsor should require before signing off.

<QuizBlock
  title="Concept check"
  intro="Three short questions on splits and leakage."
  questions={[
    {
      prompt: <>A churn model trained with a random 80/20 split shows AUC 0.92 on the test set. When deployed to score next month's customers, AUC drops to 0.71. What is the most likely diagnosis?</>,
      options: [
        { label: <>The model has overfit hyperparameters.</> },
        { label: <>The random split allowed customers to appear in both training and test slices with different timestamps; a time-based split would have caught the inflated estimate.</> },
        { label: <>The dataset is too small.</> },
      ],
      correctIndex: 1,
      explanation: <>Most churn data has within-customer correlation across time. A random row-level split lets the model effectively memorize each customer; a time-based split is the production-shaped evaluation.</>,
    },
    {
      prompt: <>Which of the following features is <em>most likely</em> to leak in a model that scores customers each Monday?</>,
      options: [
        { label: <>days_since_last_purchase, computed as of Sunday night.</> },
        { label: <>support_tickets_last_30d, computed over the past 30 days.</> },
        { label: <>refund_count_finalized, computed from invoice records that close 14 days after month-end.</> },
      ],
      correctIndex: 2,
      explanation: <>The third feature's value at Monday-morning score time is not yet available — it is computed only after invoices close. In production the column will be empty or stale; in training it was reconstructed retrospectively, so the model learned a relationship it cannot use.</>,
    },
    {
      prompt: <>A team has been tuning a model against the same held-out test set for six months. The model now shows AUC 0.04 higher than the original prototype. The honest interpretation is:</>,
      options: [
        { label: <>The improvement is real and the model is ready to ship.</> },
        { label: <>Some of the improvement may be the team having implicitly trained on the test set through repeated comparisons. A fresh held-out set should confirm the gain before deployment.</> },
        { label: <>Cross-validation would have produced the same result without the risk of peeking.</> },
      ],
      correctIndex: 1,
      explanation: <>Each comparison against the test set leaks a tiny amount of information into the choices the team makes. Over six months, that drift can mimic real improvement.</>,
    },
  ]}
/>
