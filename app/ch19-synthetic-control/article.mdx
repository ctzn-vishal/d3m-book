...
<Pitfall title="Poor pre-treatment fit">
If the treated unit's scale or dynamics sit outside the support of the donor pool — a New York City format against rural towns — the optimizer cannot find a good fit and the post-period gap is uninterpretable. **Reject the design if the pre-treatment RMSE is large relative to the typical post-treatment movement.**
</Pitfall>

<Pitfall title="Donor contamination">
If donor units are themselves affected by the treatment through migration, marketing spillovers, or policy diffusion, the synthetic counterfactual absorbs part of the treatment and attenuates the estimate toward zero. **Define the donor pool to exclude likely-contaminated units before running the optimization.**
</Pitfall>

<Pitfall title="Cherry-picking the donor pool">
An analyst can manipulate the estimated effect by adding or removing donors until the result looks the way they want. **Fix the donor pool transparently before fitting weights, and report sensitivity to alternative pools as standard.**
</Pitfall>
