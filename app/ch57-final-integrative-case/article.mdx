...
- Build the data dictionary that documents every column's source, definition, and trust level (§3.1, §3.2).
- Run the **Data Language Studio (§4.1)** on the candidate-market table to produce a clean metric brief.
...
- A **dashboard storyboard** (§8.1) that walks an executive through: *here is the candidate pool → here is where we already work → here is the overlap → here is where the white space is*.

The visual decision brief (§8.2) is the first managerial artefact: an executive-facing summary of where in the candidate pool the strongest visual cases live, *before any modelling*.
...
- **Difference-in-differences (§12.1)** on past city launches — for each historical expansion, compare the entered market's local coffee/foodservice metrics to a synthetic comparison of similar non-entered cities. Did the entry move regional indicators, or was the entered market already converging there?
- **Heterogeneous treatment effects (§12.3)** — break down past expansion outcomes by market type (college towns vs. office hubs vs. dense urban infills vs. suburban). Which types delivered the strongest first-year revenue per square foot?
- **Synthetic control (§12.2)** on a flagship past expansion — what would have happened in that market without our entry, and how confident is the team in that counterfactual?
- An **identification memo (§11.2)** documenting the assumptions: parallel trends in the donor pool, no contemporaneous shocks, the threats and how the team treats them.
...
- A **supervised model** trained on historical store-level first-year revenue, with features from demographics, geography, competitor density, and Bean &amp; Basket's online customer penetration in the area (§14, §15.3 for numeric prediction).
- A **classification companion** that scores each candidate market on the probability of clearing a profitability threshold within 18 months (§15.1, §15.2 for the threshold-profit logic).
- **Segmentation** of the candidate pool (§16.1) to group similar markets together — large coastal college towns, mid-sized Sun Belt office hubs, dense Midwest urban infills, etc. The segments will inform the *rollout sequencing*, not just the ranking.
- A **perceptual map** of candidate markets along value-vs-premium and convenience-vs-experience dimensions (§16.2), so the team can see which white spaces the firm is best positioned to occupy.
- A **model card (§15.5)** documenting the predictive model's known failure modes — particularly the markets where the historical training data is thinnest.
...
- **Sentiment and topic modelling (§18.4, §18.5)** on local Yelp / Google reviews of existing coffee shops. What do customers in this market value? What complaints recur?
- **Embedding-based clustering (§19.1)** of local review text to surface the dominant themes by market.
- **GPT-as-measurement (§19.2)** on the local review corpus, scoring constructs like "brand-as-third-place importance", "willingness to pay for premium", "loyalty to local independents", "convenience over experience."
- **Multimodal social-media monitoring (§20.4)** — image and text posts about coffee in each market. Are there visual conventions (the matcha latte aesthetic, the third-wave craft narrative) the firm should accommodate?
- A **RAG (§20.1) assistant** built on the firm's own past market-entry post-mortems, so the team can ask "what did we learn about markets that looked like Greenville?" and get cited answers from the corporate knowledge base.
...
- An **AI workflow card (§22.1)** for the per-market voice-monitoring system that runs *during* the rollout — sentiment, topic, construct, and emerging-issue tracking in each newly entered market.
- A **portfolio monitoring view (§24.2)** that tracks every new store's revenue, churn, customer voice, and operational metrics against the per-market predictions from Part IV.
...
The eleven sections of the §24.1 template, pulled from every Part above. A real memo would fit on one page; what follows is the structure.
...
- **The operating system is mostly unbuilt.** The §24.2 portfolio monitoring described above assumes a level of infrastructure most firms haven't built. The book describes the target; the firm has to fund the path.
...
<QuizBlock
  title="Concept check"
  intro="Three short questions on the integrative case."
  questions={[
    {
      prompt: <>The expansion decision touches every Part of the book. The most honest characterization of how the Parts combine is:</>,
      options: [
        { label: <>One Part dominates; the others are nice-to-have.</> },
        { label: <>Each Part contributes a piece of evidence the others cannot produce — descriptive, visual, causal, predictive, AI, operational — and the memo synthesizes them.</> },
        { label: <>The Parts are interchangeable.</> },
      ],
      correctIndex: 1,
      explanation: <>This is the book's central claim: different evidence languages answer different questions, and a hard decision usually needs more than one.</>,
    },
    {
      prompt: <>The team's Part IV model has wide confidence intervals on a particular candidate market segment because the training data in that segment is thin. The right next step is:</>,
      options: [
        { label: <>Ignore the confidence intervals.</> },
        { label: <>Phase the rollout so the thin-data segments are deferred until wave-1 outcomes generate training data to narrow the intervals.</> },
        { label: <>Build a bigger model.</> },
      ],
      correctIndex: 1,
      explanation: <>Phased rollouts are how a learning organization handles uncertainty: act where the model is confident, defer where it isn't, and use the action's outcomes to update.</>,
    },
    {
      prompt: <>The book's strongest claim about its own durability is:</>,
      options: [
        { label: <>The methods will be valid forever.</> },
        { label: <>The methods will rotate; the discipline — naming the decision, the counterfactual, the threshold; shipping the artefacts; monitoring the portfolio — will outlast the technology.</> },
        { label: <>The current AI tools are the endpoint.</> },
      ],
      correctIndex: 1,
      explanation: <>The closing rule. The frame is meant to survive multiple generations of methods.</>,
    },
  ]}
/>
