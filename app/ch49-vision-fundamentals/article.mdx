...
- **CLIP** trains an image encoder and a text encoder *jointly*, so that the embedding of "a photo of a labrador" sits near the embedding of an actual photo of a labrador. The result is a *shared* embedding space across text and images — the foundation of multimodal AI in §20.4.
...
A useful generalization: most of what you learned about embeddings in §19.1 transfers. The vision model produces vectors. Vectors near each other in the space correspond to images that are similar. Nearest-neighbour search, clustering, anomaly detection — all the patterns from text — apply to images too.
...
- **Document processing.** OCR + layout understanding. The subject of §20.3.
...
- **Train/test distribution mismatch.** The model performs well in evaluation and badly in production because the production images don't look like the training images. Domain randomization, broader data collection, and periodic re-evaluation are the standard mitigations.
- **Overfitting to incidental cues.** A defect detector learns the timestamp watermark instead of the defect. Carefully sample training data to break confounds.
- **Cost of labelling.** Bounding boxes and especially segmentation are expensive. Plan the labelling budget before committing to the output shape.
- **Latency and edge constraints.** A model that runs in a research notebook may not run on a shop-floor camera. Production vision systems often use smaller, distilled models or run on dedicated hardware.
- **Privacy in image data.** Customer-facing surveillance and analytics raise privacy obligations the rest of the analytics stack doesn't. Bake the governance in from day one.

---

## Managerial Takeaway

Vision AI works because models learn a *feature hierarchy* — edges to textures to parts to objects — that transfers across domains. CNNs and ViTs share that property; the differences are mostly engineering choices about how filters are applied. The output shape the model produces is the contract with the rest of the system: label, boxes, mask, or embedding.

> *Pick the output shape that matches the action. Use pretrained backbones to save labelling cost. Audit for subgroup bias before deploying anything customer-facing.*
...
<QuizBlock
  title="Concept check"
  intro="Three short questions on vision fundamentals."
  questions={[
    {
      prompt: <>A team wants to count SKUs on shop-floor shelf images. Which output shape is the natural choice?</>,
      options: [
        { label: <>Class label.</> },
        { label: <>Bounding boxes — one per SKU detected.</> },
        { label: <>Segmentation mask.</> },
      ],
      correctIndex: 1,
      explanation: <>Counting and localizing objects is the textbook use case for object detection (bounding boxes). Segmentation would work but is overkill given the labelling cost.</>,
    },
    {
      prompt: <>A defect detector trained in one factory works well; deployed in a second factory it performs poorly. The most likely diagnosis is:</>,
      options: [
        { label: <>The model is too small.</> },
        { label: <>Distribution shift — lighting, camera angle, or background differs between the two factories; data diversity, not model size, is usually the fix.</> },
        { label: <>The defect is different.</> },
      ],
      correctIndex: 1,
      explanation: <>This is the canonical production-vision failure. The solution is to train on data from both factories, not to make the model larger.</>,
    },
    {
      prompt: <>"Self-supervised pretraining" is most accurately described as:</>,
      options: [
        { label: <>Training without any data.</> },
        { label: <>Training a model on a task derived from unlabelled data (e.g. predicting masked patches), so that downstream fine-tuning needs only a small labelled set.</> },
        { label: <>The same as supervised learning.</> },
      ],
      correctIndex: 1,
      explanation: <>SSL extracts learning signal from the input data itself, not from labels. It shifts the labelling cost from "huge" to "small", making vision feasible in specialized domains.</>,
    },
  ]}
/>
