...
- Free-text → structured JSON (§21.2).
...
- **Output format.** The shape of the response. JSON schema, plain text, bullet list, code, etc. If you want JSON, ask for JSON — and validate (§21.2).
...
- The output format is non-negotiable. If the team will parse JSON downstream, the prompt should commit to JSON up front. §21.2 covers the structured-output mechanics that enforce this.
...
From here the chapter gets mechanical: **constrain the output** so it flows into the next system without cleanup (§21.2), **give the model tools** so it can act rather than just answer (§21.3), and **govern** the whole thing (Chapter 22).

</Takeaway>

<QuizBlock
  title="Concept check"
  intro="Three questions spanning what LLMs do and how to brief them."
  questions={[
    {
    ...
    },
    {
      ...
      correctIndex: 1,
      explanation: <>This is the GABRIEL paper's finding. Robustness to phrasing is evidence the task is well-specified — and a signal to stop polishing the prompt.</>,
    },
    {
