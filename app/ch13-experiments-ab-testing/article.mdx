...
<Pitfall title="Sample ratio mismatch">
When the actual traffic split deviates from the intended one — 52/48 instead of 50/50 — it is almost always a randomization bug or selective drop-out, not random noise. **Check the split before reading any lift; a broken split means the arms are no longer comparable.**
</Pitfall>

<Pitfall title="Early stopping">
Peeking daily and stopping the moment the result crosses the significance line inflates the apparent lift — the inflation is a mathematical consequence of selecting on a noisy walk, not evidence the effect is real. **Pre-register the stopping rule and hold to it, or use a sequential test designed for peeking.**
</Pitfall>

<Pitfall title="Interference and spillovers">
When treated users tell control users about the feature, or treated stores cannibalize demand from neighbors, the control arm is no longer a clean counterfactual and the estimated lift attenuates toward zero. **Randomize at a level above the interference — by store, region, or market — when units can influence each other.**
</Pitfall>
