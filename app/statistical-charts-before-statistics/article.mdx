...
The first error is treating the log transform as a trick. It is not a trick; it changes the question from differences in units to differences in percentages. That is often exactly what a pricing manager needs.

The second error is believing a trend line because it is smooth. A line through confounded historical data can be precise, visually compelling, and wrong for decision-making. The line summarizes the data it sees. It does not know whether the comparison is credible.

The third error is hiding the interval. A coefficient without an interval is a point pretending to be a fact. The interval reminds the reader that the estimate is a range of plausible values under the model, not a command.
