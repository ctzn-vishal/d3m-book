...
import { MetricFormulaLab } from '@/components/Book/part1/MetricFormulaLab';
import { MetricCompositionTree } from '@/components/Book/part1/DataLanguageDiagrams';
import transformsRaw from './data/transformations.json';
import metricsRaw from './data/metrics.json';
import metricLab from './data/metric-lab.json';
...
Figure 3 takes one metric apart until every leaf is a column somebody actually records. Every fork on the way down is a decision someone made.

<Figure width="page-outset" caption="Figure 3. Net revenue, decomposed to its collected columns. Nobody collects revenue — they collect line items, and someone decided that a refund subtracts, that a discount is not a separate transaction, and that an order placed at 23:58 belongs to today. When two dashboards disagree, they disagree at one of these forks.">
  <MetricCompositionTree />
</Figure>

Figure 4 is that contract, made clickable. Pick a metric and every degree of freedom the prose just warned about is on screen at once: the numerator, the denominator, the unit, the window, the owner, and the thing that will silently change the number.

<Figure width="page-outset" caption="Figure 4. Nine metric contracts. The formula is the least interesting row — a metric argument is almost never about arithmetic. It is about the window, the exclusions, and who gets to change them, which is why the owner is on the card at all.">
  <MetricFormulaLab items={metricLab.items} />
</Figure>
