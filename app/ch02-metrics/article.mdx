...
Figure 3 is that contract, made clickable. Pick a metric and every degree of freedom the prose just warned about is on screen at once: the numerator, the denominator, the unit, the window, the owner, and the thing that will silently change the number.

<Figure width="page-outset" caption="Figure 3. Nine metric contracts. The formula is the least interesting row — a metric argument is almost never about arithmetic. It is about the window, the exclusions, and who gets to change them, which is why the owner is on the card at all.">
