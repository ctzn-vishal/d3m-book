...
Choosing a baseline because it makes the story vivid — indexing to the lowest month so every recovery looks large, or the highest so every decline looks severe — produces a chart that is technically correct and editorially manipulative. **Pick the baseline that names the real comparison, not the one that maximizes the swing.**
</Pitfall>

<Pitfall title="Hiding scale behind an index">
An index can make a niche product and a flagship look equally important: both rise from 100 to 140 even if one moves ten times more units. **Pair the indexed view with absolute levels so a small mover is not mistaken for a large one.**
</Pitfall>

<Pitfall title="The baseline as causal control">
Campbell's is a useful benchmark for Progresso, but it is not automatically a counterfactual. **A benchmark clarifies the comparison; it does not, by itself, identify cause.**
</Pitfall>
