...
This article introduces the two strands of unsupervised learning that recur across the book — **clustering** (row-oriented: which observations are alike?) and **dimensionality reduction** (column-oriented: which variables are correlated?). The first half of Chapter 16 focuses on clustering for segmentation; the next two articles take up PCA, perceptual maps, and nonlinear methods.
...
- **Treating clusters as causal types.** A segment is a description of similarity, not an explanation of behaviour. A "discount-sensitive" segment is one whose discount-share is high — not one where lowering discount-share would change behaviour.
- **Re-fitting per quarter without anchoring.** If the segmentation drifts every refit, segment-level strategies cannot persist. Either pin segment definitions to fixed rules once they stabilize, or schedule a deliberate refresh cadence with a migration plan.
- **Over-targeting tiny segments.** A segment of 800 customers may have a memorable story and a vanishing budget impact. Size segments before strategizing them.
- **Forgetting the static features.** Including only behavioural features can produce segments that are stable to behavioural noise but unstable to demographic shifts. Always check static features against the clusters as a sanity test.

---

## Managerial Takeaway

Unsupervised methods do not give the business answers. They give it ways of looking. The strongest segmentation is the one that:

> *Has names a manager can repeat from memory, strategies that differ from segment to segment, and definitions that hold across two reasonable algorithm choices.*

If the segmentation cannot pass that test, the algorithm has produced clusters but not yet a segmentation.

<QuizBlock
  title="Concept check"
  intro="Three short questions on clustering for segmentation."
  questions={[
    {
    ...
    {
      prompt: <>A churn-prone "Price-Sensitive Switchers" segment has high discount share. Management proposes to "reduce discount share to lower churn." Which best describes this proposal?</>,
      options: [
        { label: <>A correct application of the segmentation.</> },
        { label: <>A causal claim about an intervention that the segmentation cannot, by itself, support. The segment describes who they are; what they would do under intervention is a Part III question.</> },
        { label: <>A retargeting strategy.</> },
      ],
      correctIndex: 1,
      explanation: <>Clusters describe similarity. Changes that follow from intervention need the identification logic of Part III, or an experiment.</>,
    },
    {
      prompt: <>K-means returns one cluster containing 60% of customers and four small clusters covering the rest. The team should consider:</>,
      options: [
        { label: <>Trying hierarchical clustering or DBSCAN — K-means assumes roughly equal-sized clusters and may have produced a "leftover" mass.</> },
        { label: <>Reducing the feature space.</> },
        { label: <>Doubling k.</> },
      ],
      correctIndex: 0,
      explanation: <>K-means handles balanced clusters well and dominant-mass distributions poorly. The shape of the cluster sizes is a signal about the algorithm choice.</>,
    },
  ]}
/>
