...
<Callout variant="caveat" title="Storing data wide is a schema trap">

The most common cost of wide-as-storage is not visible until the next period arrives. April's revenue needs an `sales_2024_04` column. Every dashboard that hard-coded `sales_2024_01`, `sales_2024_02`, `sales_2024_03` needs to be updated. Every join that referenced "the revenue column" now needs to know which one. Every regression that used the three months as predictors silently drops April because there is no `april` predictor in the model. None of this is hypothetical — it is the entire reason analytics teams reshape data to long at ingestion. A long table absorbs the new month as a row; a wide table demands a schema change.

A subtler version: **wide storage hides missing periods**. If Store C did not exist in January, the wide row simply has a blank or zero for `sales_2024_01`, which a chart engine will plot as zero — making the store look like it had revenue of $0 in January rather than "the store didn't exist yet." In long form, the missing period is a missing row, and most plotting tools draw that as a gap, which is the correct visual.
