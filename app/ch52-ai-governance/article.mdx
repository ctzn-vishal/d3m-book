...
- **Single-metric evaluation.** A workflow that ships on accuracy alone misses seven other dimensions.
- **Stale golden sets.** Evaluation data ages out. Refresh quarterly with new examples reflecting current traffic.
- **Drift unnoticed.** Without monitoring, a workflow's quality erodes silently. The first symptom is usually a customer complaint, not a dashboard alert.
- **Risk register that nobody reads.** Risks logged once and forgotten. Treat the register as a living document; review with the same cadence as the workflow card.
- **The owner has rotated.** The on-call list is six months out of date; nobody knows who responds. Treat ownership as part of the operating model, not a paperwork item.

---

## Managerial Takeaway

AI evaluation and governance is the operating system for the methods in Part V. Eight evaluation dimensions, a risk-control map, an AI workflow card, monitoring, and a named owner. Each lives in concrete artefacts the firm can produce on demand.

> *Define the rubric. Map the risks. Ship the card. Monitor weekly. Re-audit quarterly. Name the owner.*
...
<QuizBlock
  title="Concept check"
  intro="Three short questions on AI evaluation and governance."
  questions={[
    {
