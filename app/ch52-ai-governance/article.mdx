...
An AI workflow that ships without an evaluation rubric, a risk register, and a one-page governance card is not yet operational. It's a research artefact in costume. The discipline of Chapter 22 is what turns the building blocks of Part V — classification, embeddings, RAG, vision, LLMs, agents — into a system the firm can stand behind in front of an auditor, a regulator, a customer, or a board.

</DropCap>

This article gives the framework. The capstone (§22.2) applies it to a full Bean &amp; Basket Customer Voice Intelligence Studio that integrates every method in Part V.

The AI Workflow Card developed here is the fourth one-page artefact in the family that began at [§9.1](/ch11-from-metrics-to-decisions). It inherits the discipline of the Decision Question Card (action, counterfactual, threshold), the Predictive Task Contract ([§14.2](/ch26-supervised-setup)), and the Model Card ([§15.5](/ch33-automl-explainability)) — and adds the governance fields that an LLM-driven system specifically needs. The Identification Memo ([§11.2](/ch16-identification)) is the parallel artefact on the causal side; both feed the [§24.1](/ch55-decision-memos) Decision Memo that ships.
...
- **Model drift** — monitoring + retraining cadence (the §17.3 logic carried into LLM-land).
...
Every shipped workflow needs a one-page contract. The same role the model card played in §15.5, restated for AI workflows.
...
The §17.3 patterns transfer to LLM workflows with two adjustments:
...
The capstone — §22.2 — applies all of this to a final integrated workflow that combines every method in Part V into a single Bean &amp; Basket Customer Voice Intelligence Studio.
