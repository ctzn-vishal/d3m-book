...
A chart with error bars looks scientific, so the recommendation feels stronger — which is backwards. **Intervals should make the claim more inspectable, not less challengeable.**
</Pitfall>

<Pitfall title="Hiding uncertainty for a cleaner chart">
Dropping intervals makes a dashboard tile prettier and less useful at once. **When the decision turns on whether two regions, months, or segments differ meaningfully, the interval is the part the decision depends on** — keep it.
</Pitfall>
