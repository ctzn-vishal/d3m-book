...
<Pitfall title="Uneven modality coverage">
A model trained on web-scale image-text pairs can have very different coverage across modalities — audio coverage is often weaker than image coverage. **Audit the model on each modality you depend on before deploying, rather than assuming parity.**
</Pitfall>

<Pitfall title="Misleading cross-modal grounding">
A model captioning an image may invent details that aren't in the image; the same hallucination patterns from text carry over. **Treat generated captions and descriptions as claims to verify, not facts, for any high-stakes use.**
</Pitfall>

<Pitfall title="Benchmark-to-production gap">
Frontier multimodal models score well on standard benchmarks and fail in surprising ways on real business documents and noisy media. **Evaluate on your own data before trusting a leaderboard number.**
</Pitfall>
...
Multimodal AI is the architectural endgame of everything in Part V: text became vectors, images became vectors, audio became vectors, and they all share a space now, so a single language model can reason across all of them. Identify the workflows that mix modalities, use multimodal models where the integration was previously the hard part, watch the costs, and govern the privacy.
...
<QuizBlock
  title="Concept check"
  intro="Three questions spanning the chapter — RAG, document AI, and the shared-space idea behind multimodal models."
  questions={[
    {
    ...
    {
      prompt: <>The single most important tunable parameter in a production document AI pipeline is:</>,
      options: [
        { label: <>The model size.</> },
        { label: <>The confidence threshold above which extractions auto-process and below which they go to a human reviewer.</> },
        { label: <>The OCR engine choice.</> },
      ],
      correctIndex: 1,
      explanation: <>The threshold is where business cost meets model accuracy. Setting it is a decision; monitoring it is operations.</>,
    },
    {
