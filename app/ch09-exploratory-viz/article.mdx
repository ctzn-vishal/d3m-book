...
<Callout variant="caveat" title="Three dashboard pathologies every team produces once">

**The buffet dashboard.** Twelve tiles, no order, no hierarchy. Every chart is the same size; every metric is presented as if equally important. The viewer is forced to do the prioritization the dashboard was supposed to do. Fix: pick one executive question per page; if there are three questions, build three pages.

**Vanity metrics on top.** Total page views, total accounts, total impressions — large, growing, and not connected to a decision. A KPI tile should answer a question the manager would otherwise ask; if the tile is a number nobody would ever act on, it does not belong on the headline row.

**Charts without comparisons.** A single bar showing "Q1 revenue: $1.18M" is not a chart, it is a label. Every chart on a dashboard should make a comparison — to last period, to plan, to target, to a benchmark. *"$1.18M, +0.7% vs Q4"* is informative; *"$1.18M"* alone is not.
