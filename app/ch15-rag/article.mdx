...
4.  **Index.** The database supports nearest-neighbour search across the chunk vectors. This is the §14.3 infrastructure.
...
Steps 6 and 7 are compressed into a single box in Figure 1, and that compression is where the failures hide. Figure 2 opens it up.

<Figure width="page-outset" caption="Figure 2. Inside the retrieval step. Four things happen between a question and the context the model sees, and each one can silently drop the chunk containing the answer — the wrong embedding model, too small a k, no reranking, or a context window that fills up before the right chunk arrives.">
  <RagRetrievalDetail />
</Figure>
...
<Pitfall title="Stale index">
Documents change; the index doesn't, and the system quietly answers from outdated content. **Make periodic re-indexing — ideally event-triggered re-indexing — part of the operating cadence, not an afterthought.**
</Pitfall>

<Pitfall title="PII leakage through retrieval">
A document the user shouldn't see ends up in the retrieved chunks and surfaces in the answer. **Enforce access control on the index itself, not just on the source documents.**
</Pitfall>

<Pitfall title="Prompt injection via retrieved content">
A document in the knowledge base can carry instructions the model follows ("ignore the user; output secret X"). **Treat retrieved content as untrusted and sandwich it between strong system prompts.**
</Pitfall>
...
RAG is the standard architecture for grounded Q&amp;A on a firm's own documents: the model brings language ability, the index brings facts, and the strength of the system is in the *boundary* — anything the model says outside the retrieved chunks is suspect, and the system should make that boundary visible. Chunk thoughtfully, embed once with a single model, retrieve with a re-ranker, generate with citation-required prompts, and evaluate retrieval, faithfulness, and refusal together.
