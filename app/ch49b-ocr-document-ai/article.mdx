...
4.  **LLM extraction.** A language model is given the OCR'd text + the layout, and a JSON schema describing what to extract. It returns structured fields. This is the §21.2 pattern applied to documents.
...
The threshold–profit curve from §15.2 reappears here, with different units. Replacing "false positive cost" with "wrong payment cost" and "false negative cost" with "human review cost" gives the right framing.
...
- **Hallucinated fields.** The model invents a vendor name or a date that isn't on the document. Schema-strict prompting + arithmetic validation catches most cases.
- **Confidence inflation.** Modern LLMs are often over-confident. Calibration on a held-out set of human-reviewed documents is essential.
- **Layout drift.** A vendor changes their invoice template. The extraction silently degrades for that vendor. Per-vendor monitoring is worth the engineering cost.
- **Privacy.** Documents often contain PII. Redaction policies, encryption-at-rest, and processor-vs-controller agreements matter — particularly for healthcare and financial documents.
- **Audit trail.** A regulator may ask why a payment of $12,000 was authorized. Without per-extraction logs (model version, confidence, prompt, retrieved chunks if RAG was involved), the audit answer is "we don't know."

---

## Managerial Takeaway

Document AI is a mature commercial pattern. The technology — OCR plus layout plus structured extraction — is commodity at this point. The differentiator is workflow design: where the confidence threshold sits, what fields are validated automatically, who reviews the rest, and how the reviewer's decisions feed back into the model.

> *Define the schema before choosing the vendor. Set the threshold by business cost. Validate against domain rules. Route ambiguity to a human. Log everything for audit.*
...
<QuizBlock
  title="Concept check"
  intro="Three short questions on document AI."
  questions={[
    {
      prompt: <>A team wants to extract invoice line items where the description, quantity, and unit price are in adjacent columns. A pure OCR system that just reads top-to-bottom is likely to:</>,
      options: [
        { label: <>Work correctly.</> },
        { label: <>Mix columns and produce nonsensical line items, because it doesn't preserve the table layout — layout-aware document AI is the standard fix.</> },
        { label: <>Refuse to process the document.</> },
      ],
      correctIndex: 1,
      explanation: <>The whole point of modern document AI is layout awareness. OCR alone is necessary but not sufficient for any document with tabular or multi-column structure.</>,
    },
    {
    ...
    {
      prompt: <>A document extraction returns "total_due: 12000" for an invoice that actually shows "$12,000.00". A simple safeguard against unit-of-account errors is:</>,
      options: [
        { label: <>Trust the model.</> },
        { label: <>Validate by recomputing subtotal + tax = total, and flag mismatches for human review.</> },
        { label: <>Always send to a human.</> },
      ],
      correctIndex: 1,
      explanation: <>Domain-rule validation catches a class of extraction errors no general-purpose LLM-eval suite will find. It is the cheapest, highest-leverage piece of document-AI engineering.</>,
    },
  ]}
/>
