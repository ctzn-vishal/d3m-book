...
- **Soup demand turns more inelastic in winter (−2.82 vs. −2.49 non-winter).** Category directors can cite this as a reason to test countercyclical pricing — nudging prices up modestly in the season when the volume penalty for doing so is smallest. Action: let the rollout calendar follow the elasticity calendar, not the reverse. Limit: the winter/non-winter split is still a within-sample comparison, not a randomized seasonal experiment.
...
>
> **1. Recommendation.** Raise the premium oat-milk surcharge from $0.50 to $0.75 (a 5% retail price increase on a standard $5 latte), implemented first as a 30-day staggered pilot in 25 West Coast stores with 25 matched control stores, timed to the winter quarter when the packaged-soup seasonal analysis (Chapter 8.3) suggests demand runs more inelastic and a comparable increase costs the least volume. Projected category margin lift across the 400-store network if validated: roughly $14.5k per week.
>
...
The memo is one page. Every claim is supported by exactly one piece of evidence from Part III: identification from Chapter 6, elasticity from Chapter 8.1, the Lerner rule from 8.2, the seasonal pricing resolution from 8.3, cross-effects from 8.4, rollout discipline from 10.
