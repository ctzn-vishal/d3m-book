...
<Callout variant="caveat" title="The chart type is not the decision">

**Starting from the tool.** "Make a map" or "make a dashboard" is not a business question. Write the comparison first.

**Using the right chart at the wrong grain.** A county scatterplot cannot answer a household-level question. A store-month histogram cannot by itself recommend a national price policy.

**Letting analytical-looking charts overclaim.** Trend lines, coefficient plots, heatmaps, and intervals look authoritative. They still need units, denominators, definitions, uncertainty, and causal limits.
