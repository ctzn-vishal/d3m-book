...
<Pitfall title="Sarcasm and negation">
Standard sentiment classifiers fail on "I just love waiting forty minutes for cold coffee" — both VADER and many fine-tuned models miss the inversion. **On social text especially, test explicitly for sarcasm and negation rather than trusting an aggregate accuracy number.**
</Pitfall>

<Pitfall title="Domain shift">
A FinBERT model trained on 10-K filings will mishandle Twitter complaints; sentiment classifiers carry their training domain with them. **Always validate on a labelled sample of the target domain before trusting the scores.**
</Pitfall>

<Pitfall title="Class imbalance hides uselessness">
Most customer text is mildly positive, so a model trained on an 80%-positive corpus can default to "positive" everywhere, look 80% accurate, and be useless. **Evaluate per class — precision and recall on the minority classes — not overall accuracy.**
</Pitfall>
...
Text classification is supervised learning under a different feature pipeline. The two operationally important variants are *routing* (where should this go?) and *aspect-based sentiment* (which part of the experience is under stress?); the headline sentiment number is rarely the right output for action. Route the text into managerial buckets, score sentiment by aspect rather than overall, and watch the change, not the level.
