...
<Callout variant="caveat" title="Transformations are not cosmetic — they encode judgment">

The most insidious error around transformations is to call them "cleaning" and stop thinking about them. **Log-transforming revenue** is not a cleaning step; it changes what the model treats as a one-unit change (from a dollar to a percent). **Standardizing variables** before clustering changes which segments emerge — a customer-level clustering done on raw revenue will group everyone by spend; the same clustering done on standardized revenue can group by behavioral pattern. **Binning a continuous variable** loses information; sometimes that is what you want (interpretable age bands for a marketing report), but if the analysis downstream depends on the continuous variation, binning destroys the signal. Every transformation is a *choice about what to make comparable*. Document the choice; do not bury it inside a notebook.

A related error is **double-counting in a metric**. Reporting "total revenue + total tax + total tip" as a single revenue figure inflates the number. Reporting "completed orders + abandoned carts" as conversions silently rewrites the metric. The defense is the same as for grain (Chapter 1) and joins (Chapter 5): before computing, write down what one unit of the metric represents.
