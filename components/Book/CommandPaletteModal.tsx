...
/**
 * The actual search UI (framer-motion + full book index) — split out from
 * CommandPalette.tsx so it can be next/dynamic-loaded only once the palette
 * is actually opened, instead of shipping on every article page whether or
 * not anyone uses ⌘K. CommandPalette.tsx owns the always-mounted trigger
 * listener and `open` state; this component is purely presentational.
 */
export function CommandPaletteModal({ book, open, onOpenChange }: CommandPaletteModalProps) {
