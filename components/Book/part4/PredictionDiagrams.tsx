  ...
  return (
    <Card title="The predictive-modeling lifecycle">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Seven stages from business question to monitoring; a dashed feedback arrow returns to the first stage.">
        ...
        <text x={30 + cellW * 1.5} y={42} textAnchor="middle" className="fill-blue-700 text-[10px] font-semibold uppercase tracking-wide">Preparation</text>
        <text x={30 + cellW * 4} y={42} textAnchor="middle" className="fill-purple-700 text-[10px] font-semibold uppercase tracking-wide">Modeling</text>
        <text x={30 + cellW * 6} y={42} textAnchor="middle" className="fill-emerald-700 text-[10px] font-semibold uppercase tracking-wide">Evaluation &amp; Operation</text>
        ...
        {stages.map((s, i) => {
          ...
          return (
            <g key={s.label}>
              ...
              <text x={x} y={yMid + 5} textAnchor="middle" className="fill-slate-900 text-[12px] font-semibold">{i + 1}</text>
              {lines.map((ln, li) => (
                <text key={li} x={x} y={yMid + 42 + li * 12} textAnchor="middle" className="fill-slate-700 text-[10px]">{ln}</text>
              ))}
              {i < stages.length - 1 && (
        ...
        />
        <text x={W / 2} y={yMid + 88} textAnchor="middle" className="fill-amber-700 text-[10px] italic">Iterate — what we learn in production reshapes the next problem definition.</text>
      </svg>
    </Card>
  );
}
          ...
          return (
            <div key={kind} className="rounded-md border border-slate-200 p-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] uppercase tracking-wide text-slate-500">{sub}</span>
              </div>
              <ul className="mt-2 space-y-1.5">
                {rows.filter(r => r.kind === kind).map(r => (
                  <li key={r.task} className="flex items-center justify-between text-[12px]">
                    <span className="text-slate-800">{r.task}</span>
                    <span className="rounded px-1.5 py-0.5 text-[10px] font-mono text-slate-700" style={{ background: bg }}>{r.target}</span>
                  </li>
                ))}
              </ul>
            </div>
  ...
  return (
    <Card title="The test set is a rehearsal for the future">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Historical data is split into training and test; the model fits on training and is scored against held-out outcomes.">
        ...
        <text x={130} y={52} textAnchor="middle" className="fill-slate-900 text-[12px] font-semibold">Historical labelled data</text>
        <text x={130} y={68} textAnchor="middle" className="fill-slate-600 text-[10px]">past customers, known outcomes</text>
        ...
        <text x={365} y={40} textAnchor="middle" className="fill-blue-900 text-[12px] font-semibold">Training (≈70%)</text>

        <rect x={280} y={60} width={170} height={30} rx={4} fill={C.greenLight} stroke={C.green} strokeWidth={1.5} />
        <text x={365} y={80} textAnchor="middle" className="fill-emerald-900 text-[12px] font-semibold">Test (≈30%)</text>
        ...
        <text x={605} y={40} textAnchor="middle" className="fill-purple-900 text-[12px] font-semibold">Fit model</text>

        <rect x={520} y={60} width={170} height={30} rx={4} fill={C.purpleLight} stroke={C.purple} strokeWidth={1.5} />
        <text x={605} y={80} textAnchor="middle" className="fill-purple-900 text-[12px] font-semibold">Predict &amp; score</text>
        ...
        <text x={485} y={142} textAnchor="middle" className="fill-slate-900 text-[12px] font-semibold">Compare predictions to known test outcomes</text>
        <text x={485} y={158} textAnchor="middle" className="fill-slate-600 text-[10px]">If performance collapses outside training, the model overfit.</text>
    ...
    <Card title="Feature leakage gallery — would this be known at decision time?">
      <ul className="grid grid-cols-1 gap-1.5 md:grid-cols-2">
        {rows.map(r => {
          ...
          return (
            <li key={r.feature} className="flex items-start gap-2 rounded border border-slate-200 px-2 py-1.5">
              <span
              ...
              <div className="text-[12px]">
                <div className="font-mono text-slate-800">{r.feature}</div>
                <div className="text-slate-500">{r.why}</div>
              </div>
            </li>
          );
        })}
  ...
  return (
    <Card title="A feature catalog for a Bean &amp; Basket churn model">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Four feature families — recency, frequency, monetary, engagement — feeding a single model.">
        {cols.map((c, i) => {
          ...
          return (
            <g key={c.label}>
              ...
              <text x={x + colW / 2} y={42} textAnchor="middle" className="fill-slate-900 text-[12px] font-semibold" style={{ fill: c.color }}>{c.label}</text>
              <text x={x + colW / 2} y={58} textAnchor="middle" className="fill-slate-500 text-[10px] italic">{c.sub}</text>
              {c.items.map((it, ii) => (
                <text key={it} x={x + colW / 2} y={108 + ii * 18} textAnchor="middle" className="fill-slate-700 text-[10px] font-mono">{it}</text>
              ))}
        ...
        <text x={W / 2} y={244} textAnchor="middle" className="fill-slate-900 text-[12px] font-semibold">Churn probability model</text>
      </svg>
  ...
  return (
    <Card title="From probability score to action — pick a threshold, sort, intervene">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Overlapping probability distributions for churners and non-churners with a decision threshold.">
        ...
        <text x={x(threshold)} y={m.top - 8} textAnchor="middle" className="fill-amber-700 text-[10px] font-semibold">threshold = 0.5</text>
        {/* axes */}
        <line x1={m.left} y1={yBase} x2={W - m.right} y2={yBase} stroke={C.ink} strokeWidth={1} />
        <text x={m.left} y={yBase + 18} className="fill-slate-500 text-[10px]">0</text>
        <text x={x(0.5)} y={yBase + 18} textAnchor="middle" className="fill-slate-500 text-[10px]">0.5</text>
        <text x={W - m.right} y={yBase + 18} textAnchor="end" className="fill-slate-500 text-[10px]">1.0</text>
        <text x={(m.left + W - m.right) / 2} y={H - 6} textAnchor="middle" className="fill-slate-600 text-[10px]">Predicted churn probability</text>
      </svg>
  ...
  return (
    <Card title="Threshold–profit curve — the manager's lever">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Net profit per customer as a function of the action threshold.">
        ...
        <text x={x(peak.t)} y={yScale(peak.profit) - 10} textAnchor="middle" className="fill-emerald-700 text-[10px] font-semibold">peak ≈ {peak.t.toFixed(2)}</text>
        {/* x labels */}
        {[0, 0.25, 0.5, 0.75, 1].map(t => (
          <text key={t} x={x(t)} y={yBase + 14} textAnchor="middle" className="fill-slate-500 text-[10px]">{t.toFixed(2)}</text>
        ))}
        <text x={(m.left + W - m.right) / 2} y={H - 8} textAnchor="middle" className="fill-slate-700 text-[11px]">Probability threshold for intervention</text>
        {/* y labels */}
        {[-0.05, 0, 0.05, 0.1, 0.15, 0.2].map(v => (
          <text key={v} x={m.left - 8} y={yScale(v) + 3} textAnchor="end" className="fill-slate-500 text-[10px]">{`$${v.toFixed(2)}`}</text>
        ))}
        <text x={14} y={(m.top + yBase) / 2} transform={`rotate(-90 14 ${(m.top + yBase) / 2})`} textAnchor="middle" className="fill-slate-700 text-[11px]">Expected profit per customer</text>
      </svg>
  ...
  return (
    <Card title="Confusion matrix — each cell has a business cost">
      <div className="grid grid-cols-[120px_repeat(2,minmax(0,1fr))] gap-1 text-[12px]">
        <div />
        <div className="text-center text-[10px] font-semibold uppercase tracking-wide text-slate-500">Actually churned</div>
        <div className="text-center text-[10px] font-semibold uppercase tracking-wide text-slate-500">Actually stayed</div>
        {[0, 1].map(row => (
          <React.Fragment key={row}>
            <div className="flex items-center justify-end text-[10px] font-semibold uppercase tracking-wide text-slate-500">
            ...
            {[0, 1].map(col => {
              ...
              return (
                <div key={col} className="rounded-md border p-3" style={{ background: bg, borderColor: border }}>
                  ...
                  <div className="text-slate-700">{c.sub}</div>
                  <div className="mt-1 text-[11px] italic text-slate-600">{c.cost}</div>
                </div>
              );
            })}
          </React.Fragment>
    ...
    <Card title="Two evaluation views that managers actually read">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Calibration plot on the left and a lift chart on the right.">
        ...
        <g transform="translate(30,20)">
          <text x={panelW / 2} y={0} textAnchor="middle" className="fill-slate-800 text-[11px] font-semibold">Calibration</text>
          <text x={panelW / 2} y={14} textAnchor="middle" className="fill-slate-500 text-[10px]">Does 70% predicted = 70% observed?</text>
          {(() => {
            ...
            return (
              <g>
                ...
                <text x={ox - 6} y={oy + 4} textAnchor="end" className="fill-slate-500 text-[10px]">1.0</text>
                <text x={ox - 6} y={oy + innerH + 3} textAnchor="end" className="fill-slate-500 text-[10px]">0.0</text>
                <text x={ox + innerW / 2} y={oy + innerH + 18} textAnchor="middle" className="fill-slate-600 text-[10px]">Predicted probability</text>
                <text x={ox - 28} y={oy + innerH / 2} transform={`rotate(-90 ${ox - 28} ${oy + innerH / 2})`} textAnchor="middle" className="fill-slate-600 text-[10px]">Observed rate</text>
              </g>
            );
          })()}
        </g>
        ...
        <g transform={`translate(${30 + panelW + 20},20)`}>
          <text x={panelW / 2} y={0} textAnchor="middle" className="fill-slate-800 text-[11px] font-semibold">Lift / Gains</text>
          <text x={panelW / 2} y={14} textAnchor="middle" className="fill-slate-500 text-[10px]">Top-10% targets capture {`>`} 40% of churners.</text>
          {(() => {
            const innerW = panelW - 50;
            const innerH = 160;
            const ox = 40;
            ...
            return (
              <g>
                <line x1={xS(0)} y1={yS(0)} x2={xS(1)} y2={yS(1)} stroke={C.muted} strokeWidth={1} strokeDasharray="3 3" />
                <text x={xS(0.7)} y={yS(0.65)} className="fill-slate-500 text-[10px]">random</text>
                <path
                ...
                <line x1={ox} y1={oy} x2={ox} y2={oy + innerH} stroke={C.ink} strokeWidth={1} />
                <line x1={ox} y1={oy + innerH} x2={ox + innerW} y2={oy + innerH} stroke={C.ink} strokeWidth={1} />
                <text x={ox + innerW / 2} y={oy + innerH + 18} textAnchor="middle" className="fill-slate-600 text-[10px]">% customers targeted (sorted by score)</text>
                <text x={ox - 28} y={oy + innerH / 2} transform={`rotate(-90 ${ox - 28} ${oy + innerH / 2})`} textAnchor="middle" className="fill-slate-600 text-[10px]">% churners captured</text>
              </g>
            );
          })()}
        </g>
    ...
    <Card title="Actual vs predicted — where does the model fall apart?">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Scatter of actual vs predicted listing price, with reference line and a high-error tail.">
        ...
        {[0, 100, 200, 300, 400].map(v => (
          <g key={v}>
            <text x={xS(v)} y={H - m.bottom + 14} textAnchor="middle" className="fill-slate-500 text-[10px]">{`$${v}`}</text>
            <text x={m.left - 8} y={yS(v) + 3} textAnchor="end" className="fill-slate-500 text-[10px]">{`$${v}`}</text>
          </g>
        ))}
        <text x={(m.left + W - m.right) / 2} y={H - 6} textAnchor="middle" className="fill-slate-700 text-[11px]">Actual nightly price</text>
        <text x={14} y={(m.top + H - m.bottom) / 2} transform={`rotate(-90 14 ${(m.top + H - m.bottom) / 2})`} textAnchor="middle" className="fill-slate-700 text-[11px]">Predicted price</text>
        <text x={xS(380)} y={yS(80)} textAnchor="end" className="fill-rose-700 text-[10px] italic">large errors in luxury tail</text>
      </svg>
      <div className="mt-2 flex flex-wrap items-center justify-center gap-4 text-[11px] text-slate-600">
...
export function DecisionTreeDiagram() {
  ...
  const node = (cx: number, cy: number, label: string, sub?: string, color = C.blue) => (
    <g>
      ...
      <text x={cx} y={cy - 3} textAnchor="middle" className="fill-slate-900 text-[11px] font-semibold">{label}</text>
      {sub && <text x={cx} y={cy + 13} textAnchor="middle" className="fill-slate-500 text-[10px]">{sub}</text>}
    </g>
  );
  const leaf = (cx: number, cy: number, label: string, color: string) => (
    <g>
      ...
      <text x={cx} y={cy + 4} textAnchor="middle" className="fill-slate-900 text-[11px] font-semibold">{label}</text>
    </g>
  );
  const edge = (x1: number, y1: number, x2: number, y2: number, label: string) => (
    <g>
      ...
      <text x={(x1 + x2) / 2} y={(y1 + y2) / 2 - 4} textAnchor="middle" className="fill-slate-500 text-[10px]">{label}</text>
    </g>
  );
  return (
  ...
  return (
    <Card title="An ensemble is a committee of diverse trees">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Six small trees on the left feeding an average prediction on the right.">
        {Array.from({ length: trees }).map((_, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={x + 30} y={130} textAnchor="middle" className="fill-slate-500 text-[10px]">tree {i + 1}</text>
              {/* arrow to ensemble */}
        ...
        <text x={W - 90} y={H / 2 - 6} textAnchor="middle" className="fill-purple-900 text-[12px] font-semibold">Ensemble average</text>
        <text x={W - 90} y={H / 2 + 12} textAnchor="middle" className="fill-purple-700 text-[10px]">stable, harder to read</text>
      </svg>
      <p className="mt-2 text-center text-[11px] text-slate-500">
  ...
  return (
    <Card title="Bias–variance trade-off — the U-curve of validation error">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Train error falls with complexity; validation error is U-shaped.">
        ...
        <text x={xS(sweet.x)} y={yS(sweet.y) - 8} textAnchor="middle" className="fill-emerald-700 text-[10px] font-semibold">sweet spot</text>
        <text x={xS(1.4)} y={yS(0.16)} className="fill-blue-800 text-[10px]">training error</text>
        <text x={xS(7.5)} y={yS(0.55)} className="fill-rose-800 text-[10px]">validation error</text>
        <text x={xS(0.5)} y={H - m.bottom + 14} textAnchor="middle" className="fill-slate-500 text-[10px]">simple</text>
        <text x={xS(9.5)} y={H - m.bottom + 14} textAnchor="middle" className="fill-slate-500 text-[10px]">complex</text>
        <text x={(m.left + W - m.right) / 2} y={H - 8} textAnchor="middle" className="fill-slate-700 text-[11px]">Model complexity (e.g. tree depth) →</text>
        <text x={14} y={(m.top + H - m.bottom) / 2} transform={`rotate(-90 14 ${(m.top + H - m.bottom) / 2})`} textAnchor="middle" className="fill-slate-700 text-[11px]">Error</text>
      </svg>
    </Card>
  );
}
      ...
      <ul className="space-y-1.5">
        {rows.map(r => (
          <li key={r.name} className="grid grid-cols-[200px_minmax(0,1fr)_40px] items-center gap-2 text-[12px]">
            ...
            <span className="text-right tabular-nums text-slate-600">{(r.imp * 100).toFixed(0)}%</span>
          </li>
        ))}
      </ul>
      <p className="mt-3 text-[11px] text-slate-500">
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={r.k} className={i % 2 === 0 ? 'bg-slate-50' : 'bg-white'}>
                <th className="w-[160px] px-3 py-1.5 text-left align-top font-semibold text-slate-700">{r.k}</th>
                <td className="px-3 py-1.5 text-slate-700">{r.v}</td>
              </tr>
            ))}
          </tbody>
        </table>
